...
import Game.Core (GModel, class Game, class ScoreGame, Dialog(..), Mode(..), bestScore,
...
iconSelectGroup ∷ ∀msg pos ext sel. IconLabel sel ⇒ Eq sel ⇒
    GModel pos ext → String → Array sel → sel → (sel → msg) → (sel → Record I.Options → Record I.Options) → Html msg
iconSelectGroup model title values selected action optionFn =
    icongroup title $ values <#> \val →
        iconbutton model (optionFn val (iconLabel val I.defaultOptions{
            selected = val == selected
        })) [E.onClick \_ → action val]

-- | groupe d'icones à choix unique, fonction alternative 
iconSelectGroup' ∷ ∀msg pos ext sel. IconLabel sel ⇒ Eq sel ⇒
    GModel pos ext → String → sel → (sel → msg) → Array (Tuple sel (Record I.Options → Record I.Options)) → Html msg
iconSelectGroup' model title selected action values =
    icongroup title $ values <#> \(val ∧ optionFn) →
        iconbutton model (optionFn (iconLabel val I.defaultOptions{
            selected = val == selected
        })) [E.onClick \_ → action val]
...
iconSelectGroupM ∷ ∀msg pos ext t sel.
    ...
    GModel pos ext → String → Array sel → t sel → (sel → msg) → (sel → Record I.Options → Record I.Options) → Html msg
...
iconSizesGroup ∷ ∀msg pos ext. MsgWithCore msg ⇒
    GModel pos ext → Array (Tuple Int Int) → Boolean → Html msg
