...
loadFromJson' ∷ ∀pos ext. DecodeJson pos ⇒ 
                GModel pos ext → Json → GModel pos ext
...
coreUpdate (SetGridSize nbRows nbColumns customSize) = 
    ...
    setSize' model =
        if nbRows >= minrows && nbRows <= maxrows && nbColumns >= mincols && nbColumns <= maxcols then
            ...
            model
        where SizeLimit minrows mincols maxrows maxcols = sizeLimit model
...
newGameAux ∷ ∀pos ext mov. Game pos ext mov ⇒
        (GModel pos ext → GModel pos ext) → GModel pos ext → Gen (GModel pos ext)
...
newGame f = do 
    ...
    put $ if List.null (model^._history) || isLevelFinished model then
        rmodel
    else
