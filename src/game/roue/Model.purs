...
type Ext' =
  { size ∷ Int
  , rotation ∷ Int
  , --- nombre de rotations effectuées, peut-être négatif et n'est pas borné.
...
instance Game Position Ext { from ∷ Location, to ∷ Location } where
  ...
  play model move = act (model ^. _position)
    where
    act = case move of
...
instance MsgWithDnd Msg Location where
  dndmsg = DnD
...
update (DnD DropOnBoard) = modify_ \model →
  let
    model2 = model # set _dragged Nothing
  in
...
update Check = lockAction $ get >>= \st → go (st ^. _size)
  where
  go 0 = do
