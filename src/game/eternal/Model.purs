...
import Game.Core (class Game, class MsgWithCore, CoreMsg, GModel, SizeLimit(..), Mode(..), Dialog(..),
                   playA, coreUpdate, _ext, genModel, newGame, isLevelFinished,
    ...
    computerMove model
        ...
        | otherwise = case model^._arena, (model^._position).attacked of
            Just arena, Just attack →
                let graph = edgesToGraph (length (model^._graph).vertices) (model^._graph).edges
