...
import Lib.Game (class Game, State (..), Mode(DuelMode), genState, newGame', canPlay, _position, _turn)
import Lib.TwoPlayersGame (class TwoPlayersGame, computerMove')
...
nimState :: NimState
nimState = genState [] (_{mode = DuelMode }) (Ext { length: 10, nbPiles: 4 })
...
instance nimGame2 :: TwoPlayersGame (Array (Tuple Int Int)) ExtState Move where
    possibleMoves state =
        repeat2 (state^._nbPiles) (state^._length) (\{row, col} -> Move row col)
        # filter (\m -> canPlay state m)
        # sortWith \(Move pile pos) -> fromMaybe 0 $ do
            Tuple a b <- (state^._position) !! pile
            pure $ if state^._turn == 0 then a - pos else pos - b
